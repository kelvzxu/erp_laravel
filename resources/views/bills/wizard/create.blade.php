                            ...
                            <div class="form-group">
                                ...
                                <select id="client" class="form-control" v-model="form.client">
                                    ...
                                    @foreach ($partner as $row)
                                        <option value="{{ $row->id }}" {{ $row->id == $orders -> vendor ? 'selected':'' }}>{{ ucfirst($row->partner_name) }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-sm-4">
                            ...
                            <div class="row">
                                <div class="col-sm-6">
                                    ...
                                    <input type="text" class="form-control bg-white" v-model="form.purchase_date"  value="<?php echo date("Y-m-d");?>"require readonly>
                                    <p v-if="errors.purchase_date" class="error">@{{errors.purchase_date[0]}}</p>
                                </div>
                                <div class="col-sm-6">
                                    <label>Due Date</label>
                                    <input type="text" class="form-control bg-white" v-model="form.due_date" readonly>
                                    ...
                                    <td class="table-name" :class="{'table-error': errors['products.' + $index + '.name']}">
                                        <select id="product" class="form-control" v-model="product.name">
                                            ...
                                            @foreach ($product as $row)
                                                <option value="{{ $row->id }}" {{ $row->id == $data ->  ? 'selected':'' }}>{{ ucfirst($row->name) }}</option>
                                            @endforeach
                                        </select>
                                    </td>
...
@section('js')
    ...
    <script type="text/javascript">
        ...
        var date = new Date();
        m = date.addDays(7);
        var payments_date = m.getUTCFullYear() +"-"+ (m.getUTCMonth()+1) +"-"+ m.getUTCDate();
        Vue.http.headers.common['X-CSRF-TOKEN'] = '{{csrf_token()}}';

        window._form = {
            purchase_no: '',
            client: '',
            client_address: '{{$orders->partner->street}},{{$orders->partner->zip}},{{$orders->partner->city}}',
            title: '{{$orders->order_no}}',
            purchase_date: '',
            due_date: payments_date,
            discount: 0,
            products: [{
                name: '',
                price: 0,
