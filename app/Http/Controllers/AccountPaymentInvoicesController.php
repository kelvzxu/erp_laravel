...
class AccountPaymentInvoicesController extends Controller
{
    ...
    public function vendor_index()
    {
        $access=access_right::where('user_id',Auth::id())->first();
        $group=user::find(Auth::id());
        $data= account_payment_invoice::with('company','vendor','journal')->orderBy('name', 'ASC')->paginate(25);
        return view ('accounting.payments.invoice.index',compact('data','access','group'));
    }
    ...
    public function store(Request $request)
    {
        try{
            ...
            $count = account_payment_invoice::where('name','like',"%".$code_journal->code."%")->count();
            if ($count==0){
    ...
    public function view($id)
    {
        $access=access_right::where('user_id',Auth::id())->first();
        $group=user::find(Auth::id());
        $payment = account_payment_invoice::find($id);
        if ($payment->partner_type =="customer")
        {
            ...
            $data= account_payment_invoice::with('company','vendor','journal')->findOrFail($id);
    ...
    {
        try{
            $payment= account_payment_invoice::findOrFail($request->id);
            $payment->update([
                'payment_type'=>$request->payment_type,
                'payment_method_id'=>$request->payment_method,
                'partner_type'=>$request->partner_type,
